...
func (s Service) Send(to string, subject string, message string) {
	// Authentication.
	auth := smtp.PlainAuth("", "abiev.arystanbek@gmail.com", "cylrfoeselqqjxkq", "smtp.gmail.com")

	// RFC #822 Standard
	writer := multipart.NewWriter(bytes.NewBufferString(""))
	var b bytes.Buffer
	_, _ = fmt.Fprintf(&b, "From: %s\r\nTo: %s\r\nSubject: %s\r\n", "abiev.arystanbek@gmail.com", to, subject)
	...
	sender := "abiev.arystanbek@gmail.com"
	if strings.Contains(sender, "<") {
		sender = text.BetweenStrings(sender, "<", ">")
	}

	// Sending email.
	err := smtp.SendMail(fmt.Sprintf("%s:%s", "smtp.gmail.com", "587"), auth, sender, []string{to}, b.Bytes())
	if err != nil {
		log.Println(err)
		return
	}
